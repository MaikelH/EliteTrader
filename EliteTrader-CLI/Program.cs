    ...
    class Program
    {
        ...
        private static string Execute(ConsoleCommand command)
        {
            ...
            if (paramInfoList.Count() > 0)
            {
                ...
                // If a string array as argument is required we handle that is separate case here.
                if (paramInfoList.ElementAt(0).ParameterType == typeof (string[]))
                {
                    methodParameterValueList.RemoveAt(0);
                    methodParameterValueList.Insert(0, command.Arguments);
                }
                else
                {
            ...
            try
            {
                object result = typeInfo.InvokeMember(
                    command.Name,
